...
uses
  ...
  StdCtrls, Buttons, OS3MainUnit, GLOBALS, Math, OUTPUTUNIT, MATRIXLIB,
  DATAPROCS, FUNCTIONSLIB, GRAPHLib, LResources;

type DynamicCharArray = array of char;
...
LoopStart:
     ...
     begin
          ...
          begin
               if Level10OK[k,j] = 1 then
               begin
                    ExpA[k,j] := (RScrGrpCnt[k,j] * (RMHRight[k,j] + FMHRight[k,j] )) /
                        NT[k,j];
               end;
          end;
     end;
...
procedure TDIFfrm.LevelScrollScroll(Sender: TObject;
...
begin
     ...
     for i := 0 to NoItems-1 do
          AlphaRel := AlphaRel + variances[i]; // sum of item variances
