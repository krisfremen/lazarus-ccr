...
procedure TTimelineMainForm.TimeLine1DrawItem(Sender: TObject; ACanvas: TCanvas;
  Item: TJvTimeItem; var R: TRect);
var
  S: string;
begin
  ...
  //  ACanvas.Brush.Color := RGB(Random(255),Random(255),Random(255));
  ACanvas.FillRect(R);
  if TimeLine1.Images <> nil then
  begin
    TimeLine1.Images.Draw(ACanvas, R.Left, R.Top, Item.ImageIndex);
    {
        if Item.Selected then
          TimeLine1.Images.Draw(ACanvas,R.Left,R.Top,Random(TimeLine1.Images.Count))
        else
          TimeLine1.Images.Draw(ACanvas,R.Right - TimeLine1.Images.Width,R.Top,Item.ImageIndex);
        }
  end;
  ...
  //  else
  S := Item.Caption;
  //  ACanvas.Font.Color := Canvas.Brush.Color xor clWhite;
  DrawText(ACanvas.Handle, PChar(' ' + S), -1, R, DT_LEFT or DT_BOTTOM or DT_SINGLELINE);
end;
